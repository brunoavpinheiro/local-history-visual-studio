    ...
    internal class DocumentRepository
    {
        ...
        public DocumentNode CreateRevision(string filePath)
        {
            ...
            try
            {
                ...
                var dirPath = Path.GetDirectoryName(newNode.RepositoryPath);
                if (!File.Exists(dirPath))
                {
                    Debug.WriteLine(string.Format(CultureInfo.CurrentCulture, "In GetRevision() of: {0} creating {1}", ToString(), dirPath));
        ...
        public DocumentNode GetRevision(string filePath, DateTime dateTime)
        {
            ...
            var unixTime = ToUnixTime(dateTime).ToString(CultureInfo.CurrentCulture);
            var fileName = Path.GetFileName(filePath);
            var relativePath = filePath.StartsWith(SolutionDirectory)
                ? filePath.Replace(SolutionDirectory + "\\", "")
                :
            ...
            var newPath = Path.Combine(RepositoryDirectory, dirPath, unixTime + "$" + fileName);
        ...
        public List<DocumentNode> GetRevisions(string filePath)
        {
            ...
            var relativePath = filePath.StartsWith(SolutionDirectory)
                ...
                // If the path is outside the solution directory, we have to mess with the drive letter
                Path.Combine(Path.GetPathRoot(filePath).Replace(":\\", ""), filePath.Replace(Path.GetPathRoot(filePath), ""));
            var newPath = Path.Combine(RepositoryDirectory, relativePath);
            ...
            var copies = new List<DocumentNode>();
        ...
        public DocumentNode GetDocumentNode(string filePath)
        {
            ...
            var dirPath = Path.GetDirectoryName(filePath.Replace(RepositoryDirectory + "\\", ""));
            var originalPath = Path.Combine(SolutionDirectory, dirPath, fileName);
            ...
            {
                var driveLetter = dirPath.Substring(0, 1);
                var dirPart = dirPath.Substring(1);

                var tempPath = driveLetter + ":" + Path.Combine(dirPart, fileName);
            ...
            var dateTime = ToDateTime(Convert.ToInt64(parts[0]));
